 ...
 *
 * Also, pod readiness probes must take cluster health into account.
 ...
 * The health check procedure should test the `DefaultCacheManager` status.
 * Here's an example with the Java language:
 *
 * [source,java]
 * ----
 * HealthChecks checks = HealthChecks.create(vertx)
 *   .register("ispn-cluster-status", future -> {
 *     VertxInternal vertxInternal = (VertxInternal) vertx; <1>
 *     InfinispanClusterManager clusterManager = (InfinispanClusterManager) vertxInternal.getClusterManager(); <2>
 *     EmbeddedCacheManager cacheManager = (EmbeddedCacheManager) clusterManager.getCacheContainer();; <3>
 *     Health health = cacheManager.getHealth(); <4>
 *     HealthStatus healthStatus = health.getClusterHealth().getHealthStatus();
 *     Status status = new Status()
 *       .setOk(healthStatus == HealthStatus.HEALTHY) <5>
 *       .setData(JsonObject.mapFrom(health)); <6>
 *     future.complete(status);
 *   });
 * ----
 * <1> Cast the `Vertx` instance to `VertxInternal`; the latter exposes the cluster manager object
 * <2> Get the cluster manager object and cast it to `InfinispanClusterManager`
 * <3> Unwrap the cache container and cast it to `EmbeddedCacheManager`
 * <4> Get the cluster health object
 * <5> Check procedure status is `OK` only if status is `HEALTHY`
 * <6> For debugging purposes, return the cluster status as a JSON object (optional)
 *
 * The health check can be exposed through a link:../../vertx-web/$lang/[Vert.x Web] `Router`:
 *
 * [source,java]
 * ----
 * Router router = Router.router(vertx);
 * router.get("/health").handler(rc -> rc.response().putHeader(HttpHeaders.CONTENT_TYPE, "text/plain").end("OK"));
 * router.get("/ready").handler(HealthCheckHandler.createWithHealthChecks(checks));
 * ----
 *
