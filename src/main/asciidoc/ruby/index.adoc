...
----
require 'vertx/vertx'
mgr = Java::IoVertxExtClusterInfinispan::InfinispanClusterManager.new()
...
----
require 'vertx/vertx'
mgr = Java::IoVertxExtClusterInfinispan::InfinispanClusterManager.new(cacheManager)
...
The readiness probe can be implemented with link:../../vertx-health-check/ruby/[Vert.x Health Checks]:

[source,ruby]
----
require 'vertx-infinispan/cluster_health_check'
require 'vertx-health-checks/health_checks'
procedure = VertxInfinispan::ClusterHealthCheck.create_procedure(vertx, true)
checks = VertxHealthChecks::HealthChecks.create(vertx).register("cluster-health", &procedure)

----

After creation, it can be exposed over HTTP with a link:../../vertx-web/ruby/[Vert.x Web] router handler:

[source,ruby]
----
require 'vertx-web/router'
require 'vertx-health-checks/health_check_handler'
router = VertxWeb::Router.router(vertx)
router.get("/readiness").handler(&VertxHealthChecks::HealthCheckHandler.create_with_health_checks(checks).method(:handle))
...
`link:../../vertx-core/dataobjects.html#VertxOptions#set_cluster_host-instance_method[clusterHost]`.
...
[source,ruby]
----
require 'vertx-infinispan/infinispan_async_map'
infinispanAsyncMap = VertxInfinispan::InfinispanAsyncMap.unwrap(asyncMap)
keyStream = infinispanAsyncMap.key_stream()
valueStream = infinispanAsyncMap.value_stream()
entryReadStream = infinispanAsyncMap.entry_stream()
