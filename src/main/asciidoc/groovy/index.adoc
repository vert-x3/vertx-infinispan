...
[source,groovy]
----
def mgr = new io.vertx.ext.cluster.infinispan.InfinispanClusterManager()
...
[source,groovy]
----
def mgr = new io.vertx.ext.cluster.infinispan.InfinispanClusterManager(cacheManager)
...
The readiness probe can be implemented with link:../../vertx-health-check/groovy/[Vert.x Health Checks]:

[source,groovy]
----
def procedure = ClusterHealthCheck.createProcedure(vertx, true)
def checks = HealthChecks.create(vertx).register("cluster-health", procedure)

----

After creation, it can be exposed over HTTP with a link:../../vertx-web/groovy/[Vert.x Web] router handler:
...
[source,groovy]
----
def infinispanAsyncMap = InfinispanAsyncMap.unwrap(asyncMap)
def keyStream = infinispanAsyncMap.keyStream()
def valueStream = infinispanAsyncMap.valueStream()
def entryReadStream = infinispanAsyncMap.entryStream()
