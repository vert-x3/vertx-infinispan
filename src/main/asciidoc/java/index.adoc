...
If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named `vertx-infinispan-3.5.1-SNAPSHOT.jar`
...
the artifact: `io.vertx:vertx-infinispan:3.5.1-SNAPSHOT` in your project.
...
First, add the `org.infinispan:infinispan-cloud:9.2.0.Beta2` and `org.jgroups.kubernetes:jgroups-kubernetes:1.0.3.Final` dependencies to your project.
