...
If you are using Vert.x from the command line, the jar corresponding to this cluster manager (it will be named `vertx-infinispan-3.6.0-SNAPSHOT.jar`
...
the artifact: `io.vertx:vertx-infinispan:3.6.0-SNAPSHOT` in your project.
...
The readiness probe can be implemented with link:../../vertx-health-check/java/[Vert.x Health Checks].
...
The health check can be exposed through a link:../../vertx-web/java/[Vert.x Web] `Router`:
