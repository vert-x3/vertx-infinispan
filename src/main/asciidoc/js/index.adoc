...
var mgr = new (Java.type("io.vertx.ext.cluster.infinispan.InfinispanClusterManager"))();
...
----
var Vertx = require("vertx-js/vertx");
var mgr = new (Java.type("io.vertx.ext.cluster.infinispan.InfinispanClusterManager"))(cacheManager);
...
The readiness probe can be implemented with link:../../vertx-health-check/js/[Vert.x Health Checks]:

[source,js]
----
var ClusterHealthCheck = require("vertx-infinispan-js/cluster_health_check");
var HealthChecks = require("vertx-health-checks-js/health_checks");
var procedure = ClusterHealthCheck.createProcedure(vertx, true);
var checks = HealthChecks.create(vertx).register("cluster-health", procedure);

----

After creation, it can be exposed over HTTP with a link:../../vertx-web/js/[Vert.x Web] router handler:
...
router.get("/readiness").handler(HealthCheckHandler.createWithHealthChecks(checks).handle);
...
`link:../../vertx-core/dataobjects.html#VertxOptions#setClusterHost[clusterHost]`.
...
[source,js]
----
var InfinispanAsyncMap = require("vertx-infinispan-js/infinispan_async_map");
var infinispanAsyncMap = InfinispanAsyncMap.unwrap(asyncMap);
var keyStream = infinispanAsyncMap.keyStream();
var valueStream = infinispanAsyncMap.valueStream();
var entryReadStream = infinispanAsyncMap.entryStream();
